...
> 本次重大更新，聚焦于 TTS 朗读功能全面重构和右侧面板 UI 重新设计。

#### 新增功能
- **右侧面板 UI 重新设计**: 5 个功能按钮（朗读、书架、设置、标注、书签）全面升级
- **毛玻璃玻璃拟态效果**: 右侧面板采用 `backdrop-filter: blur(16px)` 半透明磨砂玻璃效果
- **Lucide SVG 图标**: 所有功能按钮替换为矢量 SVG 图标，替代原有 emoji 图标
- **卡片式设置分组**: 设置面板采用卡片式分组布局，分类更清晰
- **主题色统一**: 主色调切换为 Tailwind Blue (`#3b82f6`)，更现代简洁

#### TTS 逻辑重构
- **双引擎简化**: 移除浏览器端 `EdgeTTSBrowser` 回退方案，统一使用 Node.js 代理服务器（端口 3004）
- **智能引擎选择**: 代理可用时自动使用 Edge TTS，否则降级到浏览器 SpeechSynthesis
- **音色加载优化**: 代理可用时仅加载 Edge 音色列表，否则加载系统语音
- **错误处理增强**: 语音合成失败时显示 Toast 提示并自动重试，不再静默失败

#### 问题修复
- **TTS 幽灵链彻底修复**: 通过 `ttsGeneration` 计数器 + `stopReadAloud` 中断机制，彻底杜绝旧异步链残留
- **代理 502 SSL 错误**: 修复代理服务器配置问题，直接使用直连模式（无需代理环境变量）
- **语音合成超时**: 代理服务器增加请求超时保护，避免无限等待

#### 优化改进
- **按钮交互动效**: 右侧按钮 hover 时上浮 + 阴影 + 缩放反馈，cubic-bezier 缓动函数
- **面板过渡动画**: 右侧面板展开/收起使用 0.3s cubic-bezier 过渡
- **Glassmorphism 风格**: 全面板统一毛玻璃风格，所有背景色统一透明度 + 模糊效果
